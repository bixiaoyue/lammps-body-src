...
{
  molecular = 0;                                      // 0 = atomic, 1 = molecular system

  // size_forward and size_border set in settings(), via Body class

  comm_x_only = comm_f_only = 0;                      // 1 if only exchange x in forward comm
  size_forward = 0;                                   // # of values per atom in comm
  size_reverse = 6;                                   // # in reverse comm
  size_border = 0;                                    // # in border comm
  size_velocity = 6;                                  // # of velocity based quantities
  size_data_atom = 7;                                 // number of values in Atom line
  size_data_vel = 7;                                  // number of values in Velocity line
  xcol_data = 5;                                      // column (1-N) where x is in Atom line
...
void AtomVecBody::deep_copy_bonus(int ibonus, int jbonus)
{
  ...
  for (int i = 0; i < 4; i++)
  {
    if (bonus[jbonus].quat != NULL) bonus[jbonus].quat[i] = bonus[ibonus].quat[i];
    else printf("AtomVecBody::deep_copy_bonus(): Warning, NULL ptr for bonus.quat\n");
  }
  for (int i = 0; i < 3; i++)
  {
    if (bonus[jbonus].inertia != NULL) bonus[jbonus].inertia[i] = bonus[ibonus].inertia[i];
...
/* ----------------------------------------------------------------------
   check if icp or dcp has errors. better to call after icp->get() method
------------------------------------------------------------------------- */

void AtomVecBody::check_pools()
{
...
void AtomVecBody::data_atom(double *coord, imageint imagetmp, char **values)
{
  ...
  // the first value: atom tag
  tag[nlocal] = ATOTAGINT(values[0]);

  // the second value: atom type
  type[nlocal] = atoi(values[1]);
  if (type[nlocal] <= 0 || type[nlocal] > atom->ntypes)
    error->one(FLERR,"Invalid atom type in Atoms section of data file");

  // the third value: body flag (0: this is not a body, 1: this is a body)
  // the actual body flag value in memory: 0 -> -1, 1 -> 0
  ...
  // the fourth value: mass of the atom
                            ...
                            int *ivalues, double *dvalues)
{
  ...
  if (nlocal_bonus == nmax_bonus) grow_bonus();
  // update the index (for body[] array) in this bonus
  bonus[nlocal_bonus].ilocal = m;
  // call BodyRoundedPolyhedron::data_body(...)
  bptr->data_body(nlocal_bonus,ninteger,ndouble,ivalues,dvalues);
  // put the index (for bonus[] array) of this bonus into the body[] array
  // body[] array is basically a tracking tool to find which bonus is corresponding to a given body
  body[m] = nlocal_bonus++;
}
