...
/* ----------------------------------------------------------------------
   ...
   modeling of bacteria film. Last modified date: 07/03/2020
------------------------------------------------------------------------- */
...
#include <cstdlib>
#include <cstring>
#include "pair_body_rounded_polyhedron_agent.h"
...
PairBodyRoundedPolyhedronAgent::PairBodyRoundedPolyhedronAgent(LAMMPS *lmp) : Pair(lmp)
{
  dmax = nmax = 0;
  discrete = NULL;
  dnum = dfirst = NULL;
...
PairBodyRoundedPolyhedronAgent::~PairBodyRoundedPolyhedronAgent()
{
...
void PairBodyRoundedPolyhedronAgent::compute(int eflag, int vflag)
{
...
void PairBodyRoundedPolyhedronAgent::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairBodyRoundedPolyhedronAgent::init_style()
{
...
double PairBodyRoundedPolyhedronAgent::init_one(int i, int j)
{
  k_n[j][i] = k_n[i][j];
  k_na[j][i] = k_na[i][j];
...
void PairBodyRoundedPolyhedronAgent::body2space(int i)
{
...
void PairBodyRoundedPolyhedronAgent::sphere_against_sphere(int ibody, int jbody,
...
void PairBodyRoundedPolyhedronAgent::sphere_against_edge(int ibody, int jbody,
...
void PairBodyRoundedPolyhedronAgent::sphere_against_face(int ibody, int jbody,
 int itype, int jtype, double** x, double** v, double** f, double** torque,
 double** angmom, int evflag)
{
...
int PairBodyRoundedPolyhedronAgent::edge_against_edge(int ibody, int jbody,
...
int PairBodyRoundedPolyhedronAgent::edge_against_face(int ibody, int jbody,
  int itype, int jtype, double** x, Contact* contact_list, int &num_contacts,
  double &evdwl, double* facc)
{
...
int PairBodyRoundedPolyhedronAgent::interaction_edge_to_edge(int ibody,
...
int PairBodyRoundedPolyhedronAgent::interaction_face_to_edge(int ibody,
...
void PairBodyRoundedPolyhedronAgent::pair_force_and_torque(int ibody, int jbody,
...
/* ----------------------------------------------------------------------
  ...
  variable shift is to make the force-distance function continuous.
------------------------------------------------------------------------- */
...
// check Beroz, Farzan, et al. "Verticalization of bacterial biofilms." Nature physics 14.9 (2018): 954-960.
  ...
  double& energy, double& fpair)
{
  ...
  double e = 0;
  if (R <= 0) {                       // deformation occurs
    double d23 = (R * sqrt(-R));      // d23 < 0
    double d25 = (R * R * sqrt(-R));  // d25 > 0
    fpair = - 5.0/2.0 * kn * d23 - shift;
    e = kn * d25 + shift * R;
  } else if (R <= cut_inner) {   // not deforming but cohesive ranges overlap
    fpair = kna * R - shift;
    e = (-0.5 * kna * R + shift) * R;
...
void PairBodyRoundedPolyhedronAgent::contact_forces(int ibody, int jbody,
...
void PairBodyRoundedPolyhedronAgent::rescale_cohesive_forces(double** x,
...
void PairBodyRoundedPolyhedronAgent::sum_torque(double* xm, double *x, double fx,
...
int PairBodyRoundedPolyhedronAgent::opposite_sides(double* n, double* x0,
                                           double* a, double* b)
{
...
int PairBodyRoundedPolyhedronAgent::edge_face_intersect(double* x1, double* x2,
...
void PairBodyRoundedPolyhedronAgent::project_pt_plane(const double* q,
...
void PairBodyRoundedPolyhedronAgent::inside_polygon(int ibody, int face_index,
...
void PairBodyRoundedPolyhedronAgent::distance_bt_edges(const double* x1,
...
void PairBodyRoundedPolyhedronAgent::total_velocity(double* p, double *xcm,
...
double PairBodyRoundedPolyhedronAgent::contact_separation(const Contact& c1,
                                                     const Contact& c2)
{
...
void PairBodyRoundedPolyhedronAgent::find_unique_contacts(Contact* contact_list,
                                                     int& num_contacts)
{
  int n = num_contacts;
  for (int i = 0; i < n - 1; i++) {
...
void PairBodyRoundedPolyhedronAgent::sanity_check()
{
